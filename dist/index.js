  ...
  tools: [
    {
      ...
      inputSchema: {
        type: "object",
        properties: {
          site: { type: "string", description: "Platform/site name, e.g. hackernews, arxiv, zhihu, bilibili" },
          command: { type: "string", description: "Command to run on the site, e.g. top, search, user, trending" },
          args: { type: "array", items: { type: "string" }, description: "Positional arguments for the command" },
          options: { type: "object", additionalProperties: { type: "string" }, description: 'CLI flags as key-value pairs, e.g. {"limit": "10", "query": "rust"}' },
        },
    ...
    {
      name: "opencli_list",
      description: "List all available opencli sites and commands. Returns the full command catalog with strategies ([public], [cookie], [intercept], [ui]).",
    ...
    },
    {
      ...
      inputSchema: {
        type: "object",
        properties: {
          site: { type: "string", description: "Platform/site name, e.g. hackernews" },
          command: { type: "string", description: "Specific command name (optional — omit to get site-level help)" },
        },
        required: ["site"],
      },
    },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === "opencli_help") {
    ...
    const helpArgs = command ? [site, command, "--help"] : [site, "--help"];
    const output = await runOpencli(helpArgs);
    return { content: [{ type: "text", text: output }] };
  }

  if (name === "opencli") {
    const { site, command, args: positional = [], options = {} } = args;
    const cmdArgs = [site, command, ...positional];
